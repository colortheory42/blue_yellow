...
def main():
    ...
    if ACOUSTIC_AVAILABLE:
        ...
        # Auto-enable acoustic system
        if hasattr(acoustic, 'simple_loopback') and hasattr(acoustic.simple_loopback, 'start'):
            acoustic.enabled = True
            acoustic.simple_loopback.start()
            print("🎤 Acoustic system enabled (voice echoes active)")
        else:
            acoustic.toggle()  # Fallback to toggle method
            print("🎤 Acoustic system enabled (voice echoes active)")
